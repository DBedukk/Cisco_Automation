...
def write_to_csv(input_data):
    ...
    filename = os.path.join(OUTPUT_DIR, f'results_{timestamp}.csv')
    data = read_json(io.StringIO(json.dumps(input_data)))
...
def write_to_yaml(input_data):
    ...
    timestamp = time.strftime("%Y-%m-%d_%I-%M-%S%p_%Z", time.localtime())
    filename = os.path.join(OUTPUT_DIR, f'results_{timestamp}.yaml')
...
def write_to_text(input_data, obj_name=''):
    ...
    if obj_name == '':
        ...
        filename = os.path.join(OUTPUT_DIR, f'{obj_name}_{timestamp}.json')
    with open(filename, 'w') as f:
...
def get_network_objects(args, token, domain_uuid):
    ...
    for obj_type in ['networks', 'hosts']:
        ...
        while True:
            ...
            if response.status_code in [200]:
                ...
                print(f'!!!!!!!!!!\nError encountered when obtaining details about {obj_type} objects.\n!!!!!!!!!!\n')
                break

    object_data['networkObjects'] = all_objects
        ...
        # If data not already collected, move ahead and collect
            ...
            if response.status_code in [200]:
                if 'items' not in response.json():
                    print(f'!!!!!!!!!!\nNo devices found in FMC. Is your FTD registered?\n!!!!!!!!!!\n')
                else:
...
def create_nodeA_endpoint(args, token, domain_uuid, input_data, response_data):
    ...
    # Check if device was found and has interfaces
    if 'interfaces' not in object_data['ftdEndpoints'][input_data['device_name']]:
        print(f'!!!!!!!!!!\nDevice {input_data["device_name"]} was not found in FMC or has no interfaces. Is the FTD registered?\n!!!!!!!!!!\n')
        return None
